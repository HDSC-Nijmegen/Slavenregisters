  ...
  ####    3. Levenshtein distance name owner                        ####
  ...
  #zijn dubbelingen te gebruiken om In-event & Out-event te controleren?
  #combinatie Eigenaar, Moeder + Naam MOET identiek zijn
  
  #kijk of geboortes voor start van de serie plaats vinden
  # dit gaat waarschijnlijk NIET bij de prive-eigenaren werken
  ...
  match_between <- function(df1, df2, lev_dist_naam, lev_dist_moeder, lev_dist_eigenaar, NUMMER1, NUMMER2){
  #Match slave names
   #unique names
   ...
   #merge into one data frame
    ...
    colnames(Slave_names_matched)[1:3] <- c(paste("Naam", 1, sep="_"), paste("Naam", 2, sep="_"), "Naam_lv")
   #clean environment
    rm(l, l_Naam, LV_matrix, Slave_names_1, Slave_names_2, x)
    
    
  #Match names owners
   #unique names
    Slave_names_1 <- df1[!duplicated(df1$Eigenaar) & df1$Eigenaar!="",]
    Slave_names_2 <- df2[!duplicated(df2$Eigenaar) & df2$Eigenaar!="",]
   #produce matrix with Levenshtein distance
    LV_matrix <- stringdistmatrix(Slave_names_1$Eigenaar, Slave_names_2$Eigenaar, method = "lv")
   #Filter Levenshtein distance == x
    x <- 0
    repeat{
     #filter Lev Dist == x
      l <- as.data.frame(which(LV_matrix==x, arr.ind=TRUE))
     #list names + Lev Dist
      l <- data.frame(Naam1 = Slave_names_1$Eigenaar[l[,1]],
                      Naam2 = Slave_names_2$Eigenaar[l[,2]])
      l$LV <- x
      l <- list(l)
     #list LVdist
      if("l_Naam" %in% ls() ){
   ...
   #merge into one data frame
    ...
    colnames(Eigenaar_matched)[1:3] <- c(paste("Eigenaar", 1, sep="_"), paste("Eigenaar", 2, sep="_"), "Eigenaar_lv")
   #clean environment
    rm(l, l_Naam, LV_matrix, Slave_names_1, Slave_names_2, x)
  ...
  #combine matches
   #find matches
    colnames(df1) <- paste(c("source_order", 
                            ...
                            1, sep="_")
    df_matched <- merge(df1, Slave_names_matched, by=paste("Naam", 1, sep="_"), all=F)
    df_matched <- merge(df_matched, Eigenaar_matched, by=paste("Eigenaar", 1, sep="_"), all=F)
   #add source_order, in-event, out-event, & birth year
    colnames(df2) <- paste(c("source_order", 
                              ...
                              "Naam_vorige", "Naam_volgende"),
                            2, sep="_")
    df_matched <- merge(df_matched, df2, by=paste(c("Naam", "Eigenaar"), 2, sep="_"), all=F )
  ...
  #add flags
   ...
   #sex
    df_matched$Match_sex <- ifelse(df_matched$sex_1==df_matched$sex_2, 1, 0)
   #out_event
   ...
   #match_score
    df_matched$Match_score <- 2.5*df_matched$Match_moeder + #twice as important + tie-breaker
                                ...
                                2*df_matched$Match_sex -
                                df_matched$Out_unended
      
    
  #selections
   #select Moeder with: 1 unknown entry OR max Levenshtein 2
   ...
   #select entries with EITHER: 1 unknown date, 2 correspondening birth years, OR same lagging and leading names
    df_matched <- df_matched[which(is.na(df_matched$year_birth_1) | is.na(df_matched$year_birth_2) |
                                     ...
                                     df_matched$Naam_vorige_1==df_matched$Naam_vorige_2 & df_matched$Naam_volgende_1==df_matched$Naam_volgende_2), ]
   ...
   #mark best match source_order_1
    ...
    x <- x %>% group_by(source_order_1) %>% filter(source_order_2==min(source_order_2)) %>% ungroup()
  ...
  #match serie 3 & 4
   ...
   #match series
    Serie34 <- match_between(Serie3, Serie4, lev_dist_naam=3, lev_dist_moeder=3, lev_dist_eigenaar=3, NUMMER1=3, NUMMER2=4)
  ...
  #match serie 2 & 3
   ...
   #match series
    Serie23 <- match_between(Serie2, Serie3, lev_dist_naam=3, lev_dist_moeder=3, lev_dist_eigenaar=3, NUMMER1=2, NUMMER2=3)
  ...
  #match serie 1 & 2
   ...
   #match series
    Serie12 <- match_between(Serie1, Serie2, lev_dist_naam=3, lev_dist_moeder=3, lev_dist_eigenaar=3, NUMMER1=1, NUMMER2=2)
  ...
  #match serie 2 & 4
   #select series
    ...
    Serie4 <- df[df$Serieregister_nr==4, c("source_order", "in_event2", "Naam", "Naam_number", "Moeder", "Moeder_number", "year_birth", "Eigenaar", "sex")]
   #match series
    Serie24 <- match_between(Serie2, Serie4, lev_dist_naam=3, lev_dist_moeder=3, lev_dist_eigenaar=3, NUMMER1=2, NUMMER2=4)
  ...
  #match serie 1 & 4
   #select series
    ...
    Serie4 <- df[df$Serieregister_nr==4, c("source_order", "in_event2", "Naam", "Naam_number", "Moeder", "Moeder_number", "year_birth", "Eigenaar", "sex")]
   #match series
    Serie14 <- match_between(Serie1, Serie4, lev_dist_naam=3, lev_dist_moeder=3, lev_dist_eigenaar=3, NUMMER1=1, NUMMER2=4)
  ...
  #match serie 1 & 3
   #select series
    ...
    Serie3 <- df[df$Serieregister_nr==3, c("source_order", "in_event2", "Naam", "Naam_number", "Moeder", "Moeder_number", "year_birth", "Eigenaar", "sex")]
   #match series
    Serie13 <- match_between(Serie1, Serie3, lev_dist_naam=3, lev_dist_moeder=3, lev_dist_eigenaar=3, NUMMER1=1, NUMMER2=3)
  ...
  #write outfiles
   #csv
    write.table(Serie34, "U:/Surfdrive/Shared/shared map slavenregisters/Suriname slavenregisters/Matching/Between/3-4 matches.txt", quote=F, sep ="\t", col.names=T, row.names=F)
    write.table(Serie23, "U:/Surfdrive/Shared/shared map slavenregisters/Suriname slavenregisters/Matching/Between/2-3 matches.txt", quote=F, sep ="\t", col.names=T, row.names=F)
    write.table(Serie12, "U:/Surfdrive/Shared/shared map slavenregisters/Suriname slavenregisters/Matching/Between/1-2 matches.txt", quote=F, sep ="\t", col.names=T, row.names=F)
    write.table(Serie24, "U:/Surfdrive/Shared/shared map slavenregisters/Suriname slavenregisters/Matching/Between/1-4 matches.txt", quote=F, sep ="\t", col.names=T, row.names=F)
    write.table(Serie14, "U:/Surfdrive/Shared/shared map slavenregisters/Suriname slavenregisters/Matching/Between/2-4 matches.txt", quote=F, sep ="\t", col.names=T, row.names=F)
    write.table(Serie13, "U:/Surfdrive/Shared/shared map slavenregisters/Suriname slavenregisters/Matching/Between/1-3 matches.txt", quote=F, sep ="\t", col.names=T, row.names=F)
   #excel plantages
    ...
    write.xlsx(Plantages34, "U:/Surfdrive/Shared/shared map slavenregisters/Suriname slavenregisters/Matching/Between/Plantages/3-4 matches.xlsx", overwrite=T)
    write.xlsx(Plantages23, "U:/Surfdrive/Shared/shared map slavenregisters/Suriname slavenregisters/Matching/Between/Plantages/2-3 matches.xlsx", overwrite=T)
    write.xlsx(Plantages12, "U:/Surfdrive/Shared/shared map slavenregisters/Suriname slavenregisters/Matching/Between/Plantages/1-2 matches.xlsx", overwrite=T)
    write.xlsx(Plantages24, "U:/Surfdrive/Shared/shared map slavenregisters/Suriname slavenregisters/Matching/Between/Plantages/2-4 matches.xlsx", overwrite=T)
    write.xlsx(Plantages14, "U:/Surfdrive/Shared/shared map slavenregisters/Suriname slavenregisters/Matching/Between/Plantages/1-4 matches.xlsx", overwrite=T)
    write.xlsx(Plantages13, "U:/Surfdrive/Shared/shared map slavenregisters/Suriname slavenregisters/Matching/Between/Plantages/1-3 matches.xlsx", overwrite=T)
   #excel particulieren
    ...
    write.xlsx(Particulieren34, "U:/Surfdrive/Shared/shared map slavenregisters/Suriname slavenregisters/Matching/Between/Particulieren/3-4 matches.xlsx", overwrite=T)
    write.xlsx(Particulieren23, "U:/Surfdrive/Shared/shared map slavenregisters/Suriname slavenregisters/Matching/Between/Particulieren/2-3 matches.xlsx", overwrite=T)
    write.xlsx(Particulieren12, "U:/Surfdrive/Shared/shared map slavenregisters/Suriname slavenregisters/Matching/Between/Particulieren/1-2 matches.xlsx", overwrite=T)
    write.xlsx(Particulieren24, "U:/Surfdrive/Shared/shared map slavenregisters/Suriname slavenregisters/Matching/Between/Particulieren/2-4 matches.xlsx", overwrite=T)
    write.xlsx(Particulieren14, "U:/Surfdrive/Shared/shared map slavenregisters/Suriname slavenregisters/Matching/Between/Particulieren/1-4 matches.xlsx", overwrite=T)
    write.xlsx(Particulieren13, "U:/Surfdrive/Shared/shared map slavenregisters/Suriname slavenregisters/Matching/Between/Particulieren/1-3 matches.xlsx", overwrite=T)
    
    
  #testsets Serie34
    #write.table(dubbel, "U:/Surfdrive/Shared/shared map slavenregisters/Suriname slavenregisters/Matching/Between/Samples/3-4 dubbel.csv", quote=T, sep =",", col.names=T, row.names=F)
    write.table(Alkmaar, "U:/Surfdrive/Shared/shared map slavenregisters/Suriname slavenregisters/Matching/Between/Samples/3-4 Alkmaar.csv", quote=T, sep =",", col.names=T, row.names=F)
    write.table(Barbados, "U:/Surfdrive/Shared/shared map slavenregisters/Suriname slavenregisters/Matching/Between/Samples/3-4 Barbados.csv", quote=T, sep =",", col.names=T, row.names=F)
