  ...
  setwd("C:/Surfdrive/HDS/Klusjes 2021-01-12")
  
  #load files
  Index <- fread("20200611Index en slavenregisters combinatie_compleet.txt")
  
  #duplicate Eigenaar_Slvreg
  Index$Eigenaar <- Index$Eigenaar_Slvreg
  ...
  #check number of unique name entries
    length(Index$Eigenaar) #2,969
   #check number of unique name entries after:
    #remove double whitespace
    length(which(!duplicated(gsub("  ", " ", Index$Eigenaar)))) #2,936
    #remove leading whitespace
    length(which(!duplicated(trimws(Index$Eigenaar, "left")))) #2,936
    #remove trailing whitespace
    length(which(!duplicated(trimws(Index$Eigenaar, "right")))) #2,936
    #remove dots
    length(which(!duplicated(gsub("\\.", "", Index$Eigenaar)))) #2,932
    #delete all whitespaces
    length(which(!duplicated(gsub(" ", " ", Index$Eigenaar)))) #2,936
    #set to lower case
    length(which(!duplicated(tolower(Index$Eigenaar)))) #2,935
  ...
  #filter rechtspersonen
   ...
   #key words: en co, gemeente, grond, firma, en zonen
    ...
    Index[grepl(" en zo", Index$Eigenaar),c("Eigenaar", "temp", "Person_description")]
    ...
    Index[grepl("grond", Index$Eigenaar),c("Eigenaar", "temp", "Person_description")]
   ...
   #en zoon
    Index$Last_name <- ifelse(grepl(" en zoon", Index$Eigenaar), paste(sub(" en zoon.*", "", Index$Eigenaar), "en zoon"), Index$Last_name)
    Index$Person_description <- ifelse(grepl(" en zoon", Index$Eigenaar), "rechtspersoon", Index$Person_description)
    Index$temp <- ifelse(grepl(" en zoon", Index$Eigenaar), trimws(sub(".*en zoon", "", Index$temp),"both"), Index$temp)
   #bank
    rm_stichting(" bank")
   #firma
  ...
  #clean remaining prefixes
   ...
   #replace
    ...
    Index <- Index[,c("Volgnr_Burgerreg", "Eigenaar_Slvreg", "F_nr_ser3", "F_nr_ser4", "Evt_spelling_index", "opm", "Eigenaar", "Last_name", "First_name", "Prefix", "Birth_name", "Postfix", "Patroniem", "Person_description", "Relation_description")]
    
    write.xlsx(Index,"C:/Surfdrive/HDS/Cleaning Prive-eigenaren/Index - standardisatie - 20210222.xlsx")
