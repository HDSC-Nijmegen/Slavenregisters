...
match_between_emancipation <- function(df1, df2, lev_dist_naam, lev_dist_eigenaar){
  ...
  df_unique <- left_join(df_matched, df_identical) %>%
    ...
    select(-identical_score)
  
  
  #### step 6: add unmatched cases ####
  
  #Emancipation Register
  df_not_matched_ER <- left_join(df1, df_matched, by= c("Id_person", "Naam_1", "Naam_number_1", "B_year", "B_year2", "Place_name1", "plantation_match", "Eigenaar_1","sex_emanc")) %>%
    group_by(Id_person) %>%
    mutate(flag = n()) %>%
    ungroup %>%
  ...
  #Series 4
  df_not_matched_SR <- left_join(df2, df_matched,  by= c("source_order", "Naam_2", "Naam_number_2", "year_birth", "plantation_name", "Place_name1_match", "Eigenaar_2", "sex")) %>%
    group_by(source_order) %>%
    mutate(flag = n()) %>%
    ungroup %>%
