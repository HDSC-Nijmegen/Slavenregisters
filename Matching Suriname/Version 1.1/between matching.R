  ...
  match_between <- function(df1, df2, lev_dist_naam, lev_dist_moeder, lev_dist_eigenaar, lev_dist_laglead, NUMMER1, NUMMER2){
    ...
    #df1 <- Serie2
    #df2 <- Serie3
    #lev_dist_naam <- set_lv_dist
    #lev_dist_moeder <- set_lv_dist
    #lev_dist_eigenaar <- set_lv_dist
    #lev_dist_laglead <- set_lv_dist
    #NUMMER1 <- 2
    #NUMMER2 <- 3
  ...
  #filter rows
   ...
   #Higher restrictions on accepted levensthein distance for matches with Serie 1 and 2
    if(NUMMER1==1 | NUMMER1==2){
    df_matched <- df_matched %>% filter(nchar(Naam_1)<=3 & stringdist(Naam_1, Naam_2) <1 |
                                            nchar(Naam_1)>=4 & nchar(Naam_1)<=7  & stringdist(Naam_1, Naam_2) <2 |
                                            nchar(Naam_1)>=8  & stringdist(Naam_1, Naam_2) <3)
  ...
  #compute match score
   ...
   #### step 5: apply adaptive Levenshtein distance ####
    ...
    df_matched <- df_matched %>% distinct(source_order_1, source_order_2, .keep_all = T)
    
  
   #### step 7: structure and store data frame ####
    
   #add original names
    x <- SR[!duplicated(SR$source_order),c("source_order", "Naam_original", "Moeder_original")]
    colnames(x) <- c("source_order_1", "Naam_original_1", "Moeder_original_1")
    df_full <- merge(df_matched, x, by="source_order_1", all.x=T)
   ...
   #rename
    colnames(df_full) <- c(paste("Typeregister", NUMMER1, sep="_"), paste("Typeregister", NUMMER2, sep="_"),
                           ...
                           paste("Naam_original", NUMMER1, sep="_"), paste("Naam_original", NUMMER2, sep="_"), paste("Naam", NUMMER1, sep="_"), paste("Naam", NUMMER2, sep="_"), paste("Naam_number", NUMMER1, sep="_"), paste("Naam_number", NUMMER2, sep="_"),
                           paste("Moeder_original", NUMMER1, sep="_"), paste("Moeder_original", NUMMER2, sep="_"), paste("Moeder", NUMMER1, sep="_"), paste("Moeder", NUMMER2, sep="_"), paste("Moeder_number", NUMMER1, sep="_"), paste("Moeder_number", NUMMER2, sep="_"),
